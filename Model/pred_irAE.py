...
### main function
### pred
def pred_irAE(dat_pred,pretrained_model,sen_len,pretrained_model_path,save_checkpoint_path,with_gpu = True):
    ...
    if current_cuda:
        ...
        region_distance = torch.tensor(np.zeros(shape = (len(pred_dat),c_num))).to("cuda")
        region_similarity = torch.tensor(np.zeros(shape = (len(pred_dat),c_num))).to("cuda")
        ...
        region_distance = torch.tensor(np.zeros(shape = (len(pred_dat),c_num))).to("cpu")
        region_similarity = torch.tensor(np.zeros(shape = (len(pred_dat),c_num))).to("cpu")
    ...
    if c_num==2:   
        ...
        region_similarity.index = pred_dat.index
