...
def train_prototype_hypersphere(dat_train,pretrained_model,sen_len,pretrained_model_path,save_checkpoint_path,batch_size = 1,train_K = 8,train_Q = 8, train_episodes = 300,val_episodes = 30,val_steps = 10,with_gpu = True):
    
    # dat_train = dat_train
    
    # Load train and test dataset in the form of N way K shot tasks. 
    if len(list(set(dat_train.region)))==2:
        ...
        class GeneralDataset(data.Dataset):
            ...
            def __getitem__(self, index):
                ...
                for class_idx, class_name in enumerate(target_classes):
                    ...
                    support.append([])
                    support_mask.append([])
                    dataset = self.train_X[self.train_df['region'] == class_name].values.tolist()
    ...
    N=len(list(set(dat_train.region)))
    batch_size = batch_size
    K=train_K
    Q=train_Q
    ...
    ref_all = dat_train[dat_train.region == list(dat_train.region.value_counts().index)[dat_train.region.value_counts().to_list().index(max(dat_train.region.value_counts()))]]
    
    for c in range(0,len(list(set(dat_train.region)))):
        if c == list(dat_train.region.value_counts().index)[dat_train.region.value_counts().to_list().index(max(dat_train.region.value_counts()))]:
            ref_all = ref_all
        else:
            ref_tmp = dat_train[dat_train.region == c]
            while len(ref_tmp)<max(dat_train.region.value_counts()):
                ref_tmp = pd.concat([ref_tmp, ref_tmp], axis=0) 
            ref_tmp = ref_tmp.iloc[range(0,max(dat_train.region.value_counts()))]
            ref_all = pd.concat([ref_all,ref_tmp], axis=0)
    
    use_gpu = current_cuda
    if use_gpu:
        support_region_nonregion = torch.tensor(np.zeros(shape = (max(dat_train.region.value_counts()),1,1,len(list(set(dat_train.region))),256))).to("cuda")
    else:
        support_region_nonregion = torch.tensor(np.zeros(shape = (max(dat_train.region.value_counts()),1,1,len(list(set(dat_train.region))),256))).to("cpu")
    
    with torch.no_grad():
        for j in tqdm(range(0,max(dat_train.region.value_counts()))):
            support = []
            support_mask = []
            for c in list(set(dat_train.region)):
                support.append([])
                support_mask.append([])
                support_irAE, support_irAE_mask = tokenizer(ref_all[ref_all.region==c].iloc[j].sentence)
            ...
            support = support.view(-1, len(list(set(dat_train.region))), 1, 256)  # [B, N, K, D]
    ...
    for c in list(set(dat_train.region)):
        support_tmp_mean = torch.tensor(np.zeros(shape = (sum(dat_train.region == c),len(support_region_nonregion[0][0][0][0])))).to("cpu")
        for n in tqdm(range(0,sum(dat_train.region == c))):
            tmp = support_region_nonregion[n][0][0][c].to("cpu")
            support_tmp_mean[n] = tmp
        support_tmp_mean = support_tmp_mean[1:].mean(0, keepdim=True)
        if c == 0:
            support_region_nonregion_mean =  np.array(support_tmp_mean).copy()
        else:
            support_region_nonregion_mean = np.concatenate((support_region_nonregion_mean,support_tmp_mean),axis = 0)
            
    with open(save_checkpoint_path+"/train_support_region_nonregion_vec.pkl","wb") as f:
        dill.dump(support_region_nonregion_mean,f)
    
    if use_gpu:
        support_region_nonregion_mean =  torch.tensor(np.array([[[support_region_nonregion_mean]]])).to("cuda")
    else:
        support_region_nonregion_mean =  torch.tensor(np.array([[[support_region_nonregion_mean]]])).to("cpu")
    
    if use_gpu:
        predict_label = torch.tensor(np.zeros(shape = (len(dat_train)))).to("cuda")
        region_distance = torch.tensor(np.zeros(shape = (len(dat_train),len(list(set(dat_train.region)))))).to("cuda")
        region_similarity = torch.tensor(np.zeros(shape = (len(dat_train),len(list(set(dat_train.region)))))).to("cuda")
        ...
        region_distance = torch.tensor(np.zeros(shape = (len(dat_train),len(list(set(dat_train.region)))))).to("cpu")
        region_similarity = torch.tensor(np.zeros(shape = (len(dat_train),len(list(set(dat_train.region)))))).to("cpu")
        query_train = torch.tensor(np.zeros(shape = (len(dat_train),256))).to("cpu")
        
    support = support_region_nonregion_mean[0]
    ...
    with torch.no_grad():
        for i in tqdm(range(0,len(dat_train))):
            ...
            query = query.unsqueeze(2).expand(-1, -1, len(list(set(dat_train.region))), -1)  # [B, totalQ, N, D]
            ...
            region_similarity[i] = similarity
            
    region_distance = pd.DataFrame(np.array(region_distance.cpu().numpy()))
    region_distance.index = dat_train.index
    region_similarity = pd.DataFrame(np.array(region_similarity.cpu().numpy()))
    region_similarity.index = dat_train.index
    if len(list(set(dat_train.region)))==2:
        region_distance.columns = ["Non-irAE Distance","irAE Distance"]
        region_similarity.columns = ["Non-irAE Cosine similarity","irAE Cosine similarity"]
        dat_train = pd.concat([dat_train,region_distance],axis=1)
        dat_train = pd.concat([dat_train,region_similarity],axis=1)
        ...
        region_distance.columns = ['Distance ' + str(i) for i in range(0,len(list(set(dat_train.region))))]
        region_similarity.columns = ['Cosine_similarity ' + str(i) for i in range(0,c_num)]
        dat_train = pd.concat([dat_train,region_distance],axis=1)
        dat_train = pd.concat([dat_train,region_similarity],axis=1)
        dat_train["pred_label"] = predict_label.cpu().tolist()
        #dat_train["relative_distance"] = dat_train["irAE Distance"] - dat_train["Non-irAE Distance"]
